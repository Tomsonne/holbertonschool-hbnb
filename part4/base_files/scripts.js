            ...
            form.addEventListener('submit', async (event) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        window.history.replaceState({}, '', `place.html?id=${placeId}`);
                    } else {
                        const data = await response.json();
                        alert('❌ Failed to submit review: ' + (data.error || response.statusText));
                    }
                } catch (error) {
                    console.error('Erreur lors de la soumission du review:', error);
...
function displayPlaces(places) {
    ...
    places.forEach(place => {
        ...
        div.innerHTML = `
            ...
            <a href="place.html?id=${place.id}" class="view-details-button">View details</a>
        `;
...
function displayPlaceDetails(place) {
    ...
    details.innerHTML = `
        ...
        <h4>Reviews</h4>
        <ul>${(place.reviews || []).map(r => `<li>${r.user}: ${r.text} (${r.rating}⭐)</li>`).join('')}</ul>
    `;
}